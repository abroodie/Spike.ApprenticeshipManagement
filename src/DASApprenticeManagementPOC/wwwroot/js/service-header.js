...
function CrossServiceHeader ($module) {
    this .$header = $module this.$navigation = $module.querySelectorAll("[data-one-login-header-nav]") this.$header.classList.add('js-enabled')
}
/**
* Initialise header
*
...
for (var i = 0; i < this.$navigation.length; i++) {
    ...
    $nav.$menuButton = $nav.querySelector('.js-x-header-toggle') $nav.$menu = $nav.$menuButton && $nav.querySelector( '#' + $nav.$menuButton.getAttribute('aria-controls') ) if (!$nav.$menuButton || !$nav.$menu)
...
CrossServiceHeader.prototype.handleMenuButtonClick = function () {
    this .isOpen = !this.isOpen this.$menuOpenClass && this.$menu.classList.toggle(this.$menuOpenClass, this.isOpen) this.$menuButtonOpenClass && this.$menuButton.classList.toggle(this.$menuButtonOpenClass, this.isOpen) this.$menuButton.setAttribute('aria-expanded', this.isOpen) if (this.$menuButtonCloseLabel && this.$menuButtonOpenLabel)
